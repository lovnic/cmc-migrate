   ...
   public function __construct( $str = array() ) {	
    ...
    public static function get_migrations( $per_page = 5, $page_number = 1 ) {
		...
		if( $_REQUEST['location'] == 'remote' ){
			$url = cmc_migrate::get_setting('remote_migration_url'); if( empty($url) ) return array();
			$token = cmc_migrate::get_setting('remote_migration_token');
			$url = add_query_arg( array('cmcmg'=>'migrations', 'token'=>$token, 'paged'=>empty($_REQUEST['paged'])? 1: $_REQUEST['paged']), $url );
			...
			$bid = cmc_migrate::get_current_blog_id(); //get_current_blog_id();
			...
			foreach( $current_page_files as $file ){
				...
				$item['filesize'] = cmc_migrate::filesize($file);
    ...
    function column_filename( $item ) {
        ...
        $actions = []; 
		if( $_REQUEST['location'] == 'remote'){
			$actions['import'] = "<a href='?page=cmcmg&cmcmg_action=remote_import$bid&XDEBUG_SESSION_START&id=$fname&_wpnonce=$nonce' >Import</a></span>";
		}else{
			$actions['delete'] =  "<a onclick='return cmcmg.delete_export();' href='?page=cmcmg$bid&cmcmg_action=delete&XDEBUG_SESSION_START&id=$fname&_wpnonce=$nonce' style='color:red;' >Delete</a></span>";
			//$actions['download'] = "<a href='?page=cmcmg$bid&cmcmg_action=download&XDEBUG_SESSION_START&id=$fname&_wpnonce=$nonce' target='_blank' > Download</a>";
			//$actions['restore'] = "<a onclick='return cmcmg.restore();' href='?page=cmcmg&cmcmg_action=restore&XDEBUG_SESSION_START&id=$fname&_wpnonce=$nonce' target='_blank' style='color:green;' > Restore</a></span>";
			$url = CMCMG_EXPORT_URL . cmc_migrate::get_current_blog_id().'/'.$fname;
			$actions['download'] = "<a href='$url' target='_blank' > Download</a>";
			$actions['restore'] = "<a href='?page=cmcmg$bid&tab=restore&id=$fname' style='color:green;' > Restore</a></span>";
		}
